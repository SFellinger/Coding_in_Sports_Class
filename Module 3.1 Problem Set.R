...
PitherDataCarson <- PitcherData[PitcherData$Pitcher == "Sentz, Carson"]

# 2) The average Velocity for Carson Sentz (For each TaggedPitchType)
PitcherDataVelo <- PitcherDataCarson[, .(                
  AvgVelo = mean(RelSpeed)),
  by=.(TaggedPitchType)]

# 3) A tally of Pitcher, TaggedPitchType, and Batter faced
TestTrack1 <- TestTrackMan%>%group_by(Pitcher, TaggedPitchType,BatterId)%>%tally()
